defmodule PhxTemplate18DevWeb.UserAuthTest do
  use PhxTemplate18DevWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias PhxTemplate18Dev.Accounts
  alias PhxTemplate18Dev.Accounts.Scope
  alias PhxTemplate18DevWeb.UserAuth

  import PhxTemplate18Dev.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PhxTemplate18DevWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PhxTemplate18DevWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        PhxTemplate18DevWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
